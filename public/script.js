...
function validate(){
  ...
  for(const link of getLinks()){
    if(!link.label||!link.url||!validUrl(link.url)){
...
async function submitForm(event){
  ...
  if(!CONFIG.API_URL||CONFIG.API_URL.includes("YOUR_DEPLOYMENT_ID")){
    notify("Configure the Apps Script API URL in public/script.js first.",true);
    return;
  }
  ...
  try{
    ...
    if(!data.success){
      throw new Error(data.error||"Unable to create the card.");
    }

    if(!data.guid){
      throw new Error("Card was created but no card ID was returned.");
    }
...
function detectLocation(){
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    ()=>{
      status.textContent=
        "Could not detect location. Enter a Google Maps URL manually.";
    },
    {
...
function loadDraft(){
  try{
    const draft=
      JSON.parse(
        localStorage.getItem("vcardDraft")||"null"
      );
